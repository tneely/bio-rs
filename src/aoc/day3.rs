...
items in each of its two compartments, so the first half of the characters represent items in the first compartment,
...
fn get_letter_score(letter: char) -> usize {
    return ALPHABET.find(letter).unwrap() + 1;
}
